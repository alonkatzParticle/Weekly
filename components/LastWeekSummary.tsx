...
import { DropboxPreviewModal } from '@/components/DropboxPreviewModal'
...
export function LastWeekSummary({
  ...
  if (!tasksLoaded)
    return <div className="animate-pulse text-sm text-muted-foreground p-4">Loading tasks...</div>
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">Last Week Summary</CardTitle>
            <Button
              variant="ghost"
              size="sm"
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">Last Week's Tasks ({tasks.length})</CardTitle>
            {onRefreshTasks && (
              <button
            ...
            <div className="space-y-2">
              {tasks.map(task => (
                <div
                  ...
                  className={`flex items-center gap-3 py-2 border-b last:border-0 ${task.dropbox_link ? 'cursor-pointer hover:bg-muted/50 -mx-1 px-1 rounded' : ''}`}
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {task.monday_url && (
                      <a href={task.monday_url} target="_blank" rel="noopener noreferrer" title="Open in Monday.com" onClick={e => e.stopPropagation()} className="text-muted-foreground hover:text-foreground transition-colors">
                        <ExternalLink className="h-3.5 w-3.5" />
                      </a>
                    )}
                    {task.dropbox_link && (
                      <a
                        href={task.dropbox_link}
                        target="_blank"
